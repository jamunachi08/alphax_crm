...
_DEDUP_EMAIL_FIELD = {"Lead": "email_id", "AlphaX PreLead": "email_id", "AlphaX Smart Lead": "email"}
...
def check_duplicate(email=None, mobile_no=None, exclude=None):
    ...
    PreLead / AlphaX Smart Lead records matching either value, excluding
    ...
    for dt, email_field in _DEDUP_EMAIL_FIELD.items():
        ...
        if not or_filters:
            continue
        title_field = "lead_name" if dt == "Lead" else ("prospect_name" if dt == "AlphaX PreLead" else "organization")
