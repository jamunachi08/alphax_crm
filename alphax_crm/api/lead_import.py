...
Territory, Industry Type, AlphaX Prospect Status, ...) and lets the user
...
to the current schema, which added Link-typed fields to AlphaX Prospect
...
three targets are supported here:
    ...
    AlphaX Prospect   — calling list; operator qualifies to Lead later
    Lead              — created directly
    AlphaX Smart Lead — canonical entry; syncs itself into a Lead on insert

Wired from the AlphaX Prospect list view (alphax_prospect_list.js). Files
...
TARGET_DOCTYPES = ("AlphaX Prospect", "Lead", "AlphaX Smart Lead")
DEFAULT_TARGET = "AlphaX Prospect"
...
# company name is "company_name" on Prospect/Lead but "organization" on
...
NAME_ALIASES = (
    ...
    # doctype (e.g. a Lead-shaped export dropped onto a Prospect import) —
...
def _execute(file_url, decisions, skip_duplicates, run_ai, user, notify=False,
    ...
    prospect_meta = frappe.get_meta("AlphaX Prospect")
    ...
    for idx, r in enumerate(rows, start=2):
        ...
        try:
            ...
            elif target == "AlphaX Prospect" and prospect_meta.has_field("import_batch"):
